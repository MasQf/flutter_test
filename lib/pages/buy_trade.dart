...
class BuyTradePage extends StatefulWidget {
  const BuyTradePage({super.key});

  @override
  State<BuyTradePage> createState() => _BuyTradePageState();
}

class _BuyTradePageState extends State<BuyTradePage> {
  final UserController userController = Get.find<UserController>();
  final TradeController tradeController = Get.put(TradeController());

  String status = '';

  Future<void> loadBuy({String status = ''}) async {
    await tradeController.loadBuyList(buyerId: userController.id.value, status: status);
  }
        ...
        children: [
          Obx(
            () => Container(
              ...
              child: tradeController.buyList.isEmpty
                  ? Center(
                      child: Text(
                        '暂无交易',
                  ...
                  : ListView.builder(
                      itemCount: tradeController.buyList.length,
                      itemBuilder: (context, index) {
                        TradeModel item = tradeController.buyList[index];
                        return Container(
                          width: 1.sw,
                          padding: EdgeInsets.fromLTRB(0, 40.h, 40.w, 40.h),
                          margin:
                              EdgeInsets.fromLTRB(0, 10.h, 0, index == tradeController.buyList.length - 1 ? 150.h : 0),
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  item.seller != null && item.seller?.avatar != ''
                                      ? Container(
                                          width: 90.w,
                                          height: 90.w,
                                          decoration: BoxDecoration(
                                            ...
                                            image: DecorationImage(
                                                image: NetworkImage(
                                                  replaceLocalhost(item.seller!.avatar),
                                                ),
                                                fit: BoxFit.cover),
                                          ),
                                        )
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      ...
                                      if (item.status == 'Pending')
                                        CupertinoButton(
                                          onPressed: () async {
                                            await TradeApi.updateStatus(tradeId: item.id, status: 'Completed');
                                            await loadBuy(status: status);
                                          },
                                          padding: EdgeInsets.zero,
                                          child: Container(
