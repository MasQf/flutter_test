...
class _ItemListPageState extends State<ItemListPage> {
  ...
  void _scrollListener() {
    // 当滚动到底部时加载更多数据
    // 将阈值调整为更大，确保在接近底部时就开始加载
    if (_scrollController.position.pixels >=
        _scrollController.position.maxScrollExtent - 300.h) {
      if (hasMoreData && !isLoading && widget.listType == ListType.latest) {
        print(
    ...
    try {
      switch (widget.listType) {
        ...
        case ListType.viewRank:
          await itemController.loadRankingLists();
          break;
      }
    } catch (e) {
      print("加载数据错误: $e");
  ...
  Future<void> loadMoreData() async {
    if (isLoading || !hasMoreData || widget.listType != ListType.latest) return;
    ...
    try {
      currentPage++;
      final initialCount = itemController.fullLatestList.length;
      ...
      if (initialCount == itemController.fullLatestList.length) {
        ...
        print(
            "加载了更多数据: ${itemController.fullLatestList.length - initialCount}项");
      }
    } catch (e) {
      print("加载更多数据错误: $e");
    } finally {
  ...
  List<ItemModel> get currentList {
    switch (widget.listType) {
      ...
      case ListType.favoriteRank:
        return itemController.favoriteRankingList;
      case ListType.viewRank:
        return itemController.viewRankingList;
    }
  }
          ...
          children: [
            CupertinoScrollbar(
              ...
              child: CustomScrollView(controller: _scrollController, slivers: [
                ...
                if (!isLoading &&
                    !hasMoreData &&
                    widget.listType == ListType.latest)
                  SliverToBoxAdapter(
                    child: Padding(
                      padding: EdgeInsets.symmetric(vertical: 30.h),
                      child: Center(
