...
class UserApi {
  ...
  // 上传背景图片
  static Future<bool> uploadBackground(
      {required String userId, required String background}) async {
    try {
      final response = await Api().post('/upload_background', data: {
        'userId': userId,
  ...
  static Future<bool> updateUsername(
    ...
    try {
      final response = await Api().post('/update_username', data: {
        'userId': userId,
        'name': name,
      });
